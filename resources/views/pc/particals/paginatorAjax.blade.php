    ...
    <ul class="pagination" role="navigation">
        ...
        @else
            <li class="paginate_button">
                <a class="paginate_button" rel="prev" aria-label="@lang('pagination.previous')" data-p="{{$paginator->currentpage()-1}}">上一页</a>
            </li>
        @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="paginate_button active" aria-current="page"><a class="paginate_button" data-p="{{$page}}">{{ $page }}</a></li>
                    @else
                        <li class="paginate_button"><a class="paginate_button" href="javascript:void(0)" data-p="{{$page}}">{{ $page }}</a></li>
                    @endif
                @endforeach
            @endif
        @endforeach
        ...
        @if ($paginator->hasMorePages())
            <li class="paginate_button">
                <a class="paginate_button" data-p="{{$paginator->currentpage()+1}}" rel="next" aria-label="@lang('pagination.next')">下一页</a>
            </li>
        @else
